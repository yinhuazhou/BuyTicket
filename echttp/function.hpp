#pragma once
#ifndef _ECHTTP_FUNCTION_
#define _ECHTTP_FUNCTION_
#include <sstream>
#include <fstream>
...
string Utf8Encode(const string& szToEncode)
{
    return boost::locale::conv::to_utf<char>(szToEncode,"gb2312");
}

string Utf8Decode(const string& szToDecode)
{
...
string substr(const string &str,string sStart,string sEnd)
{
    if(str=="") {
        return "";
    }
    string midStr=str.substr(str.find(sStart)+sStart.length());

    if(midStr.find(sEnd)!=string::npos) {
...
string GetCurrentTimeGMT()
{
    ...
    format_date.imbue(locale(format_date.getloc(),tfacet));
    format_date<<now;
    return format_date.str();
}

string DateFormat(string date_str,string format_str)
{
    using namespace boost::gregorian;
    date d;
    ...
    locale fmt(locale::classic(),new boost::gregorian::date_facet(format_str.c_str()));
...
string Date2UTC(string time_str)
{
    return DateFormat(time_str,"%a %b %d 00:00:00 UTC+0800 %Y");
}

vector<string> explode(string strs,string delimiter)
{
    vector<string>  strAry;
    while(strs.find("#")!=string::npos) {
        string tick=strs.substr(0,strs.find("#"));
...
string GetFormInputValue(string strs,string name)
{
    string midstr=strs.substr(strs.find(name)+name.size());
    return substr(midstr,"value=\"","\"");
}

string FileContentType(string path)
{
    map<string,string> typeAry;
