...
boost::shared_ptr<respone> client::send()
{
    try {
        ...
        while (!m_task.is_end) {
            vector<char> buf=m_task.get_write_data(m_buffer_size);
        ...
        return syn_read_body();

    } catch(const boost::exception& ex) {
        stop();
        m_respone->error_code=101;
        m_respone->error_msg="connect and send data error";
        return m_respone;
    }
}
...
void client::handle_resolver(boost::system::error_code err, tcp::resolver::iterator endpoint_iterator)
{
    if(!err) {
        ...
        stop();
        m_respone->error_code=err.value();
        m_respone->error_msg=err.message();
        mHttpBack(m_respone);
    }
}
...
void client::handle_header_read(boost::system::error_code err,size_t bytes_transfarred)
{
    if(!err) {
        ...
        string header(boost::asio::buffers_begin(bufs),boost::asio::buffers_begin(bufs) + m_header_size);
        respone_.consume(m_header_size);

        if(!m_respone->parse_header(header)) {
            stop();
            m_respone->error_code=1;
            m_respone->error_msg="解析http header错误!";
            mHttpBack(m_respone);
        }
        ...
        if(m_respone->header.find("Content-Length")!="") {
            string len=m_respone->header.find("Content-Length");
...
void client::handle_chunk_read(boost::system::error_code err,size_t bytes_transfarred)
{
    if(!err||err.value()==2) {
        boost::asio::streambuf::const_buffers_type bufs = respone_.data();
        vector<char>  readData(boost::asio::buffers_begin(bufs),boost::asio::buffers_end(bufs));
        respone_.consume(respone_.size());
        m_chunk_reader.push(readData);
        vector<char> chunk_data=m_chunk_reader.get();
        ...
        if (!m_chunk_reader.m_chunk_end) {
            if(protocol_==1) {
                boost::asio::async_read(
                    ssl_sock,respone_,boost::asio::transfer_at_least(1)
                    ,boost::bind(&client::handle_chunk_read,this,boost::asio::placeholders::error
                                 ,boost::asio::placeholders::bytes_transferred));

            } else {
                boost::asio::async_read(
                    socket_,respone_,boost::asio::transfer_at_least(1)
                    ,boost::bind(&client::handle_chunk_read,this,boost::asio::placeholders::error
                                 ,boost::asio::placeholders::bytes_transferred));
            }
        } else {
...
void client::handle_body_read(boost::system::error_code err,size_t bytes_transfarred)
{
    if(!err||err.value()==2) {
        ...
        boost::asio::streambuf::const_buffers_type bufs = respone_.data();
        vector<char> read_buf(boost::asio::buffers_begin(bufs),boost::asio::buffers_begin(bufs)+buf_size);
        respone_.consume(buf_size);

        m_respone->save_body(read_buf);

        m_respone->notify_status(2,m_body_size,m_respone->length);
...
boost::shared_ptr<respone> client::syn_read_body()
{
    if(protocol_==1) {
        ...
        string header=reader.syn_read_header();
        m_respone->parse_header(header);
        if (m_respone->header.find("Content-Length")!="") {
            size_t total_size=atoi(m_respone->header.find("Content-Length").c_str());
            size_t read_size=0;
            ...
            while (read_size<total_size) {
                ...
                vector<char> buf=reader.syn_read(read_num);
            ...
            while (!reader.m_chunk_end) {
                vector<char> buf=reader.syn_read_chunk_body();
                m_respone->save_body(buf);
            }
        }
    } else {
        ...
        m_respone->parse_header(header);
        if (m_respone->header.find("Content-Length")!="") {
            size_t need_read_size=atoi(m_respone->header.find("Content-Length").c_str());
            size_t read_size=0;

            while (read_size<need_read_size) {
                vector<char> buf=reader.syn_read(m_buffer_size);
                m_respone->save_body(buf);
                read_size+=buf.size();
